...
*/
#ifndef HEURISTICS_VAR_BIPARTITE_GRAPH_PARTITIONER
#define HEURISTICS_VAR_BIPARTITE_GRAPH_PARTITIONER

#include "../utils/equiv.hh"

class VarBipartiteGraphPartitioner : public PartitionerInterface
{
  // private:
public:
  Solver &s;
        ...
        for(int i = 0 ; cover && i<occVar[v].size() ; i++)
          {
            ...
            for(int j = 0 ; cover && j<c.size() ; j++) cover = partvec[mapVar[var(c[j])]] != grp || present[var(c[j])];             
          }
...
public:
  VarBipartiteGraphPartitioner(Solver &_s, OccurrenceManagerInterface *_om): s(_s), om(_om)
  {
    assert(om);
    ...
    xpins = (int *) malloc((om->getNbClause() + 1) * sizeof(int));
    vwghts = (int *) malloc((om->getNbClause() + 1) * sizeof(int));
    
    markedClauses.initialize(om->getNbVariable(), false);
    weightClause.initialize(om->getNbClause(), 1);            
    partvec = (int *) malloc((om->getNbVariable() + 1) * sizeof(int));
  ...
  ~VarBipartiteGraphPartitioner(){free(xpins); free(pins); free(cwghts); free(vwghts); free(partvec); free(partweights);}
